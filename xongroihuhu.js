    ...
    for (var i = 0; i < files.length; i++) {
        ...
        reader.onload = function(event) {
            ...
            imgObj.onload = function() {
                ...
                canvas.add(image);
                imageObjects.push(image); // Add image to the array
                canvas.renderAll();
            };
        };
...
canvas.on('object:selected', function(e) {
    ...
    // Ensure label is brought to front when shape is selected
    if (selectedObject.label) {
        canvas.bringToFront(selectedObject.label);
    }
});
*/
/*
canvas.on('selection:cleared', function() {
    // Clear selection handling, ensure label visibility if needed
});
*/
